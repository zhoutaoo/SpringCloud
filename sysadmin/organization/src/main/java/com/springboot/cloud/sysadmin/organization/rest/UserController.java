...
public class UserController {
	 ...
	 * @return
	 */
	@ApiOperation(value = "查询用户", notes = "根据用户名或者手机号，简单查询")
	@ApiImplicitParam(paramType = "query", name = "value", value = "用户名或者手机号", required = true, dataType = "string")
	@ApiResponses(@ApiResponse(code = 200, message = "处理成功", response = Result.class))
	@GetMapping(value = "queryByUsernameOrMobile")
	public Result queryByUsernameOrMobile(@RequestParam String value) {
		log.debug("query with value:{}", value);
		return Result.success(userService.getByUsernameOrMobile(value));
	}
}